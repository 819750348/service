    ...
    <el-container>
      <el-aside width="150">
        <el-tabs v-model="searchCategory" tab-position="left" @tab-click="tabClick">
          <el-tab-pane label="全部模型" name="0">
          </el-tab-pane>
          <el-tab-pane v-for="item in categorys" :label="item.name" :name="item.id.toString()" :key="item.id">
          </el-tab-pane>
        </el-tabs>
      </el-aside>
      <el-main>
          <el-form>
            <el-form-item>
                <el-input v-model="searchKeys" placeholder="名称/标签/上传人员/上传部门/描述" @keyup.enter="fetchData(1)">
            ...
            <el-form-item>
                <el-table :data="models" :show-header="false" :fit="true">
                    <el-table-column min-width="140">
                        ...
                        <div :class="'el-icon-sm-category'+scope.row.category"></div>
                        </template>
                    </el-table-column>
                    <el-table-column min-width="500">
                        ...
                        <div>
                            ...
                            >{{scope.row.modelName}}</router-link>
                              ...
                              <div class="canshu">
                                <router-link :to="'/model-approval-process-'+scope.row.id+'-0'">查看流程</router-link>
                              </div>
                            </div>
...
export default {
  ...
  methods:{
    fetchData(pageNum){
      var vm = this;
      this.$axios.post("/model/myCompletedWarehousing?pageNum="+pageNum+"&category="+this.searchCategory,{keys:this.searchKeys})
      .then(data => {

    ...
    <el-container>
      <el-aside width="150">
        <el-tabs v-model="activeName" tab-position="left" @tab-click="tabChange">
          ...
          <el-tab-pane label="完整模型" name="complete"></el-tab-pane>
          <el-tab-pane label="模型组件" name="component"></el-tab-pane>
          <el-tab-pane label="模型数据" name="data"></el-tab-pane>
        </el-tabs>
      </el-aside>
      <el-main>
            ...
            <template slot-scope="scope">
              ...
              <div>
                <div style="width:100%; float:left">
                  <div class="Biaoti" style="margin-right:5px;">
                    <router-link
                      v-if="scope.row.modelStatus < 5"
                      :to="'/model-resource-edit-'+scope.row.id+'-0'"
                  ...
                  <el-tag type="danger" size="mini" v-if="scope.row.modelStatus < 5">草稿</el-tag>
                  <el-tag type="success" size="mini" v-else>已提交</el-tag>
                  <div class="canshu">
                    <router-link
                      v-if="scope.row.modelStatus >= 5"
                      :to="'/model-approval-process-'+scope.row.id+'-0'"
                    >查看流程</router-link>
                  </div>
                </div>
                <hr>
...
export default {
  ...
  methods: {
    fetchData(pageNum) {
      var vm = this;
      this.$axios
        .get("/model/myUploads?pageNum=" + pageNum + "&pageSize=" + vm.pageSize)
        .then(data => {
